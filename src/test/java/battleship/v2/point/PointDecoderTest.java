...
import battleship.v2.point.PointDecoder;
...
class PointDecoderTest {
    ...
    void shouldEqualsWhenOtherIsTheSameFromInput(int x, int y, String input) throws MalformedException {
    ...
    void shouldNotEqualsWhenOtherIsNotTheSameFromInput(int x, int y, String input) throws MalformedException {
        Point point = new Point.Builder(x, y).build();
        Point pointDecoded = PointDecoder.inputToPoint(input);
    ...
    void shouldEqualsWhentherIsTheSameFromInput(String x, int y) throws MalformedException {
        int row = PointDecoder.getRow(x);
        assertEquals(y, row);
    }
    @Test
    void shoultNotEqualsWhenStringIs2andExpected0() throws MalformedException {
        int row = PointDecoder.getRow("a2");
        assertNotEquals(row, 0);
    }
    @ParameterizedTest(name = "Is  the same X(row) when given String as row")
    @CsvSource({ "a1,0", "b2,1", "c3,2", "D4,3", "E5,4", "F6,5", "g7,6", "h8,7", "i9,8", "J10,9" })
    void shouldEqualWhenIsTheSameStringFromInput(String x, int y) throws MalformedException {
        int row = PointDecoder.getColumn(x);
        assertEquals(y, row);
    }
    @Test
    void shoultNotEqualsWhenStringIsCandExpected1() throws MalformedException {
